...
struct Settings {
    ...
    bool startWindows=false, startMinimized=false, minimizeToTray=false;
};
...
void SetDesktopUi(bool desktop){
    ...
    MoveWindow(H(IDC_MINTRAY),rightX+395,r.bottom-66,20,22,TRUE);
    MoveWindow(GetWindow(H(IDC_MINTRAY),GW_HWNDNEXT),rightX+419,r.bottom-65,135,22,TRUE);
...
void BuildControls(){
    ...
    Add(L"BUTTON",L"",BS_AUTOCHECKBOX,rightX+395,r.bottom-66,20,22,IDC_MINTRAY);
    Add(L"STATIC",L"Minimize to tray",0,rightX+419,r.bottom-65,135,22,0);
...
LRESULT CALLBACK Proc(HWND w,UINT m,WPARAM wp,LPARAM lp){switch(m){case WM_GETMINMAXINFO:{auto* mmi=(MINMAXINFO*)lp;mmi->ptMinTrackSize.x=920;mmi->ptMinTrackSize.y=660;return 0;}case WM_SHOW_EXISTING_INSTANCE:ShowMain();return 0;case WM_UPDATE_AVAILABLE:ShowUpdateAvailable((UpdateInfo*)lp);return 0;case WM_CREATE:gWnd=w;BuildControls();RefreshList();LoadSelected();SetTimer(w,1,250,nullptr);return 0;case WM_SIZE:
