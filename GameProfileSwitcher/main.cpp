...
bool IsRunning(const std::wstring&name){ if(name.empty())return false; HANDLE h=CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS,0); if(h==INVALID_HANDLE_VALUE)return false; PROCESSENTRY32W e{sizeof(e)}; bool found=false; if(Process32FirstW(h,&e))do{ std::wstring n=e.szExeFile; auto d=n.find_last_of(L'.'); if(d!=std::wstring::npos)n.resize(d); if(_wcsicmp(n.c_str(),name.c_str())==0){found=true;break;} }while(Process32NextW(h,&e)); CloseHandle(h); return found; }
void CheckProcesses(){ if(gPaused)return; GameProfile* hit=nullptr; for(auto& p:gSettings.profiles) if(p.enabled&&IsRunning(ProcessName(p.exePath))){hit=&p;break;} std::wstring next=hit?hit->name:gSettings.desktop.name; if(next!=gActive){ if(hit)Apply(*hit); else Apply(gSettings.desktop); gActive=next; InvalidateRect(gWnd,nullptr,FALSE);} }
...
void SetDesktopUi(bool desktop){
    ...
    MoveWindow(H(IDC_APPLY),rightX+115,yButtons,105,36,TRUE);
...
void BuildControls(){ RECT r;GetClientRect(gWnd,&r); int rightX=298,rightW=r.right-318;
 ...
 Add(L"BUTTON",L"Apply now",BS_PUSHBUTTON,rightX+115,530,105,36,IDC_APPLY);
 ...
 Add(L"BUTTON",L"Restore Desktop",BS_PUSHBUTTON,32,r.bottom-119,221,36,IDC_RESTORE);
...
void ResizeControls(){
    ...
    MoveWindow(H(IDC_RESTORE),32,r.bottom-119,221,36,TRUE);
